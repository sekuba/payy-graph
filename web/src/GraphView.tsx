...
export function GraphView({ graph, focus, onSelect }: Props) {
  ...
  return (
    <div ref={container} className="relative h-full w-full overflow-hidden">
      ...
      <Legend truncated={graph.truncated} />
...
function Node({
  ...
  const { txn, group } = node
  const kind = KIND[txn?.kind ?? 1]
...
function groupDetail(group: Group): string {
  const parts: string[] = []
  if (group.leaving) parts.push(`${group.leaving} leave`)
  if (group.unspent) parts.push(`${group.unspent} unspent`)
  return parts.join(', ') || 'click to expand'
}

/** Deposits show their depositor, withdrawals their recipient */
function nodeLabels(graph: Graph): Map<string, string> {
  const labels = new Map<string, string>()
  for (const d of graph.deposits) {
    labels.set(d.txHash, d.label ?? shortHex(d.depositor, 6))
  }
  for (const w of graph.withdrawals) {
    labels.set(w.txHash, w.label ?? shortHex(w.recipient, 6))
  }
  return labels
}

function Legend({ truncated }: { truncated: boolean }) {
  return (
    <div
      className="absolute top-3 right-3 flex items-center gap-4 text-xs"
...
function Tooltip({ hover }: { hover: Hover }) {
    ...
    if (group) {
      ...
      return (
        <div className="tooltip" style={style}>
          ...
          <div style={{ color: 'var(--ink-2)' }}>
            {group.leaving} notes leave this view, {group.unspent} unspent
          </div>
  ...
  const n: NoteEdge = hover.edge.note
  return (
    <div className="tooltip" style={style}>
      <div>
      ...
      <div style={{ color: 'var(--ink-2)' }}>
        {n.to ? (n.continues ? 'spent outside this view' : 'spent') : 'unspent'}
      </div>
      <div className="mono" style={{ color: 'var(--muted)' }}>
...
function amountText(n: NoteEdge): string {
  ...
  if (n.max !== undefined) return `${usdc(n.min)} to ${usdc(n.max)}`
  return `at least ${usdc(n.min)}`
}
